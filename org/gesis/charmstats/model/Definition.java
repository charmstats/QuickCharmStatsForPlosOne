 ...
 *
 */
public class Definition extends DBEntity {
	
	/* charmstats.definitions (MySQL)
		`id`			 	int(11)		  NOT NULL auto_increment,
		`text`			 	varchar(2048) 		   default NULL,
		`object_entity`	 	int(11)				   default NULL,
	...
	*/
	...
	private String resize(String text) {
		return text.substring(0, java.lang.Math.min(this.getText().length(), MAX_SIZE));
	}
	...
	public boolean entityStore(Connection connection) {
		...
		if (entity_id > 0) {
			...
			try {
				...
				sqlQuery = "UPDATE definitions SET "
					+ "text = ?, "
					+ "object_entity = ?, "
					+ "object_entry = ? "					
					+ "WHERE id = ?";
				...
				prepStmt.setString(1, DBField.DEF_TXT.truncate(this.getText()));
			...
			try {
				...
				if (storeStatus) {
					 ...
					 */		
					sqlQuery = "INSERT INTO definitions (text, object_entity, object_entry ";
					sqlQuery += ") VALUES ("
						+ "?, ?, ?";
					sqlQuery += ")";
