 ...
 *
 */
public class KeywordRef extends DBReference {

	/* charmstats.keyword_refs (MySQL)
		`entity`			smallint(5) unsigned NOT NULL default '0',
		`entry`				int(11)				 NOT NULL default '0',
		`keyword`			int(11)	   			 NOT NULL,
		`ref_relationship`	int(11)	   					  default NULL,
		PRIMARY KEY  (`entity`,`entry`,`keyword`)
	*/
	
	/*
	 *	Column names and other text constants
	 */
	public static final String table_name = 	"keyword_refs";
	...
	public static final String cn_key_id =		"keyword";
	...
	public KeywordRef () {
		...
		entity_type = EntityType.KEYWORDS_REFERENCES;
	 ...
	 * @param referenced
	 */
	public KeywordRef (Object referenced) {
		this();
	...
	public int getEntityTypeID() {
		...
		if (getReferenced() instanceof DBEntity )
			entity_type_id = ((DBEntity)getReferenced()).getEntityType().getID();
	...
	public int getEntryID() {
		...
		if (getReferenced() instanceof DBEntity )
			entry_id = ((DBEntity)getReferenced()).getEntityID();	
	 ...
	 * @param dsKeywordID
	 */
	public void setKeywordID(int dsKeywordID) {
		this.dsKeywordID = dsKeywordID;
	}

	/**
	 * @return
	 */
	public int getConSpecificationID() {
		return dsKeywordID;
	}
	...
	public boolean entityLoad(Connection connection) {
		...
		try {
			if(!connection.isClosed()) {
				...
				String sqlQuery = "SELECT keyword " +
					"FROM keyword_refs " +
					"WHERE keyword=\"" + this.getKeyword().getEntityID() + "\"" +
					"AND table_id=\"" + this.getEntityTypeID()+ "\"" +
					"AND table_entry_id=\"" + this.getEntryID() + "\"";
	...
	public boolean entityStore(Connection connection) {
		...
		if ((this.getKeyword().getEntityID() > 0) && 
			...
			if (!this.entityLoad(connection)) {
				...
				try {
					...
					if (storeStatus) {
						 ...
						 */		
						sqlQuery = "INSERT INTO keyword_refs (entity, entry, keyword, ref_relationship";
						sqlQuery += ") VALUES ("
							+ "'"	+ this.getEntityTypeID()	+ "',"
							+ "'"	+ this.getEntryID()	+ "',"
							+ "'"	+ this.getKeyword().getEntityID()	+ "',"
    ...
    public boolean entityRemove(Connection connection) {
		...
		try {
			sqlQuery = "DELETE from "+ table_name +" "+
				"WHERE " + cn_key_id +" = "+ this.getKeyword().getEntityID();
