...
public class ImportReportFrame extends InternalFrame {
...
//	public static final String CONTROL_BOX_ICON	= "org/gesis/charmstats/Resources/report.GIF";
	...
	public static final String TEMPLATE	= "import_template"; 
...
//        saveBtn.addActionListener(al);
	...
	private void buildReport(ResourceBundle resourceBundle, Object addenda) {
		statistics	= (ImportStatistics)addenda;
		
		template = resourceBundle.getString(TEMPLATE);
			
		template = insertUserName(template, statistics.getImportingAdmin());
		template = insertDateOfImport(template, statistics.getImportingDate());
		template = insertSource(template, statistics.getImportedDatafile());
			
		template = insertListOfSelections(template, statistics.getSelection());
		template = insertListOfImportedOnes(template, statistics.getImported(), statistics.getOriginalNames(), statistics.getOriginalLabels()); 
		template = insertListOfDuplicates(template, statistics.getDuplicated());
		template = insertListOfSkippedOnes(template, statistics.getSkipped());
		template = insertListOfUnhandledOnes(template, statistics.getUnhandled());
	...
	private String insertDateOfImport(String template, Timestamp importDate) {
		...
		return template.replaceAll("#DATE_OF_IMPORT#", timeString);
	}
	
	private String insertSource(String template, String fileName) {
		
		return template.replaceAll("#FILENAME#", (fileName.isEmpty() ? "-" : fileName));
	}
...
//	private String insertListOfImportedOnes(String template, List<Attributes> attributes) {
//		String attributeList = getFormatedAttributeList(attributes);
	private String insertListOfImportedOnes(String template, List<Attributes> attributes, List<String> oldNames, List<String> oldLabels) {
		String attributeList = getFormatedAttributeListWithOldNameNLabels(attributes, oldNames, oldLabels);
		
		return template.replaceAll("#LIST_OF_IMPORTS#", (attributeList.isEmpty() ? "-" : attributeList)); 
	}
	
	private String insertListOfDuplicates(String template, List<Attributes> attributes) {
		String attributeList = getFormatedAttributeList(attributes);
		
		return template.replaceAll("#LIST_OF_DUPLICATES#", (attributeList.isEmpty() ? "-" : attributeList)); 
	}
	
	private String insertListOfSkippedOnes(String template, List<Attributes> attributes) {
		String attributeList = getFormatedAttributeList(attributes);
		
		return template.replaceAll("#LIST_OF_SKIPPED_ONES#", (attributeList.isEmpty() ? "-" : attributeList)); 
	}
	
	private String insertListOfUnhandledOnes(String template, List<Attributes> attributes) {
		String attributeList = getFormatedAttributeList(attributes);
		
		return template.replaceAll("#LIST_OF_UNHANDLED_ONES#", (attributeList.isEmpty() ? "-" : attributeList)); 
	}
	...
	private String getFormatedAttributeListWithOldNameNLabels(List<Attributes> attributes, List<String> oldNames, List<String> oldLabels) {
		String attributeList = new String();
		
		Iterator<Attributes> attrIterator = attributes.iterator();
		
		Iterator<String> oldNameIterator = oldNames.iterator(); 
		Iterator<String> oldLabelIterator = oldLabels.iterator();
		
		while (attrIterator.hasNext()) {
			Attributes attr = attrIterator.next();
			String oldName = oldNameIterator.next();
			String oldLabel = oldLabelIterator.next();
			
			if (attr instanceof Variable) {
				Variable var = (Variable)attr;
				
				attributeList += var.getName() +" - "+ var.getLabel() + 
					( ((var.getName().equals(oldName)) && (var.getLabel().equals(oldLabel))) ? "" : " (orig. "+ oldName +" - "+ oldLabel +")" ) +"<BR>\n"; 
			}
			if (attr instanceof Measurement) {
				Measurement mea = (Measurement)attr;
				
				attributeList += mea.getName() +" - "+ mea.getLabel() + 
						( ((mea.getName().equals(oldName)) && (mea.getLabel().equals(oldLabel))) ? "" : " (orig. "+ oldName +" - "+ oldLabel +")" ) +"<BR>\n"; 
