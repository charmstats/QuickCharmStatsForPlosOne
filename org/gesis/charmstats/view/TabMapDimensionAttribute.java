 ...
 *
 */
public class TabMapDimensionAttribute extends Tab {
	 ...
	 * @param locale
	 */
	public TabMapDimensionAttribute(Locale locale) {
		super(locale);
		
		setName("TabMapDimensionAttribute");
	}
	 ...
	 * @param locale
	 */
	public TabMapDimensionAttribute(CStatsModel model, ActionListener al, Locale locale) {
		this(locale);
		...
		/* Setup Buttons */
		backButton.setActionCommand(ActionCommandText.BTN_CON_STP_MAP_ATR_BACK);
		backButton.addActionListener(al);
//		resetButton.setActionCommand(ActionCommandText.BTN_CON_STP_MAP_ATR_RESET);
//		resetButton.addActionListener(al);
		noteButton.setActionCommand(ActionCommandText.BTN_CON_STP_MAP_ATR_NOTE);
		noteButton.addActionListener(al);
		nextButton.setActionCommand(ActionCommandText.BTN_CON_STP_MAP_ATR_NEXT);
	...
	private JComponent buildRecodeCLPanel(final CStatsModel model, WorkStepInstance layer, ResourceBundle bundle, ArrayList<Variable> measurements, ArrayList<ConDimension> dimensions) {
		 ...
		 * 
		 */
		final AttributeMapTable mapTable = new AttributeMapTable(model, new ConDimension(), new Variable(), resourceBundle);
		...
		instanceComboBox.setName("instanceSelector");
		 ...
		 */
		map0Table = mapTable;
		mapTable.addRows(model.getProject(), AttributeMapType.SPECIFICATION);
	...
	public void fillModel(CStatsModel model) {
		...
		instanceComboBox.addItem(BLANK_STRING);
		ArrayList<InstanceMap> maps = model.getProject().getContent().getInstanceMapsByType(InstanceMapType.CONCEPTUAL);		
		...
		map0Table.addRows(model.getProject(), AttributeMapType.SPECIFICATION);
