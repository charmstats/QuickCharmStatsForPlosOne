 ...
 *
 */
public class TabMapIndicatorAttribute extends Tab {
	 ...
	 * @param locale
	 */
	public TabMapIndicatorAttribute(Locale locale) {
		super(locale);
		
		setName("TabMapIndicatorAttribute");
	}
	 ...
	 * @param locale
	 */
	public TabMapIndicatorAttribute(CStatsModel model, ActionListener al,
			Locale locale) {
		this(locale);
		...
		JPanel recodeOLPanel = (JPanel)buildRecodeOLPanel(model.getProject(), null, resourceBundle, new ArrayList<ConDimension>(), new ArrayList<OperaIndicator>());
		
		/* Add Form Components to Form Panel */
		formPanel.add(recodeOLPanel);
		
		/* Setup Buttons */
		backButton.setActionCommand(ActionCommandText.BTN_OPE_STP_MAP_ATR_BACK);
		backButton.addActionListener(al);
//		resetButton.setActionCommand(ActionCommandText.BTN_OPE_STP_MAP_ATR_RESET);
//		resetButton.addActionListener(al);
		noteButton.setActionCommand(ActionCommandText.BTN_OPE_STP_MAP_ATR_NOTE);
		noteButton.addActionListener(al);
		nextButton.setActionCommand(ActionCommandText.BTN_OPE_STP_MAP_ATR_NEXT);
	 ...
	 * @return
	 */
	private JComponent buildRecodeOLPanel(Project project, WorkStepInstance layer, ResourceBundle bundle, ArrayList<ConDimension> dimensions, ArrayList<OperaIndicator> indicators) {
		 ...
		 * 
		 */
		final AttributeMapTable mapTable = new AttributeMapTable(model, new OperaIndicator(), new ConDimension(), resourceBundle);
		 ...
		 */
		map0Table = mapTable;
		mapTable.addRows(project, AttributeMapType.ASSIGNED_INDICATOR);
	...
	public void fillModel(CStatsModel model) {
		...
		instanceComboBox.addItem(BLANK_STRING);
		ArrayList<InstanceMap> maps = model.getProject().getContent().getInstanceMapsByType(InstanceMapType.OPERATIONAL);		
		...
		map0Table.addRows(model.getProject(), AttributeMapType.ASSIGNED_INDICATOR);
