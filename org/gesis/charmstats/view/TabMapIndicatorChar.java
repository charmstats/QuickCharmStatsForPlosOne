 ...
 *
 */
public class TabMapIndicatorChar extends Tab {
	 ...
	 * @param locale
	 */
	public TabMapIndicatorChar(Locale locale) {
		super(locale);
		
		setName("TabMapIndicatorChar");
	}
	 ...
	 * @param locale
	 */
	public TabMapIndicatorChar(CStatsModel model, ActionListener al, Locale locale) {
		this(locale);
		...
		JPanel recodeOLPanel = (JPanel)buildRecodeOLPanel(model, null, resourceBundle, new ArrayList<ConDimension>(), new ArrayList<OperaIndicator>());
		
		/* Add Form Components to Form Panel */
		formPanel.add(recodeOLPanel);
		
		/* Setup Buttons */
		backButton.setActionCommand(ActionCommandText.BTN_OPE_STP_MAP_CHA_BACK);
		backButton.addActionListener(al);
		backButton.setEnabled(false);
//		resetButton.setActionCommand(ActionCommandText.BTN_OPE_STP_MAP_CHA_RESET);
//		resetButton.addActionListener(al);
		noteButton.setActionCommand(ActionCommandText.BTN_OPE_STP_MAP_CHA_NOTE);
		noteButton.addActionListener(al);
		confirmButton.setVisible(true);
		confirmButton.setEnabled(true);	
		confirmButton.setActionCommand(ActionCommandText.BTN_OPE_STP_MAP_CHA_CONFIRM);
		confirmButton.addActionListener(al);
		nextButton.setEnabled(false);
		nextButton.setActionCommand(ActionCommandText.BTN_OPE_STP_MAP_CHA_NEXT);
	 ...
	 * @return
	 */
	private JComponent buildRecodeOLPanel(final CStatsModel model, WorkStepInstance layer, ResourceBundle bundle, ArrayList<ConDimension> dimensions, ArrayList<OperaIndicator> indicators) {
		 ...
		 * 
		 */
		final CharacteristicMapTable recodeTable = new CharacteristicMapTable(model, new OperaPrescription(), new ConSpecification(), resourceBundle);	
		...
		recodeTable.addRows(model.getProject(), CharacteristicMapType.PRESCRIPTION);
	...
	public void fillModel(CStatsModel model) {
		...
		instanceComboBox.addItem(BLANK_STRING);
		ArrayList<InstanceMap> instMaps = model.getProject().getContent().getInstanceMapsByType(InstanceMapType.OPERATIONAL);		
		...
		ArrayList<AttributeMap> attrMaps = model.getProject().getContent().getMapsByType(AttributeMapType.ASSIGNED_INDICATOR);
		...
		recode0Table.addRows(model.getProject(), CharacteristicMapType.PRESCRIPTION);
